...
async def password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    password = utils.generate_password(length, use_special)
...
async def username(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username = utils.generate_username(style)
    await update.message.reply_text(
...
async def uuid_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = utils.generate_uuid(version)
    await update.message.reply_text(
...
async def random_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    number = utils.generate_random_number(min_val, max_val)
    await update.message.reply_text(
        f"🎲 **Random Number:**\n`{number}`\n\nRange: {min_val} - {max_val}",
        parse_mode='Markdown'
    )

async def random_string(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    random_string = utils.generate_random_string(length, include_special)
    await update.message.reply_text(
...
async def hash_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    algorithms = utils.list_hash_algorithms()
    parts = text.split()
    algorithm = 'sha256'
    ...
    hash_value = utils.generate_hash(text, algorithm)
    await update.message.reply_text(
        f"🔐 **Hash Generated:**\nAlgorithm: {algorithm}\nText: `{text}`\nHash: `{hash_value}`",
        parse_mode='Markdown'
    )
...
async def timestamp_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    timestamp = utils.timestamp_generator(format_type)
...
def main():
    ...
    try:
        ...
        application.add_handler(CommandHandler("string", random_string))
        ...
        application.add_handler(CommandHandler("timestamp", timestamp_command))
